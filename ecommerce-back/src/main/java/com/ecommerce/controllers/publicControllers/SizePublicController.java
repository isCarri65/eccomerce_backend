...
public class SizePublicController {
    ...
    public SizePublicController(SizeService service) {
        this.service = service;
    }

    @GetMapping
    public ResponseEntity<Set<Size>> getAllActives() {
        return ResponseEntity.ok(service.getAllActives());
    }

    @GetMapping("/{id}") // Doble llave para escapar en format()
    public ResponseEntity<Size> getByIdActives(@PathVariable Long id) {
